<template>
  <div class="comparison-page">
    <!-- Header -->
    <div class="header">
      <h2 class="title">比较大小题目生成</h2>
      <p class="subtitle">支持整数、小数、分数比较大小</p>
    </div>
    ...
    <div v-if="wrongQuestionStats && isSubmitted" class="analysis-section">
      <ComparisonWrongQuestionAnalysis
...
// modify by jx: implement comparison page with control panel, action buttons, question display, answering and scoring

import { ref, computed, watch } from 'vue';
import { ElMessage } from 'element-plus';
import ComparisonControlPanel from '@/components/ComparisonControlPanel.vue';
import ComparisonDisplay from '@/components/ComparisonDisplay.vue';
import ActionButtons from '@/components/ActionButtons.vue';
import ScoreDisplay from '@/components/ScoreDisplay.vue';
import ComparisonWrongQuestionAnalysis from '@/components/ComparisonWrongQuestionAnalysis.vue';
import ComparisonTutoringPlan from '@/components/ComparisonTutoringPlan.vue';
import { useComparisonGenerator } from '@/composables/useComparisonGenerator';
import { useExport } from '@/composables/useExport';
import { validateComparisonAnswer } from '@/utils/comparisonUtils';
import { calculateScore } from '@/composables/useScoring';
import { extractComparisonWrongQuestions, calculateComparisonWrongQuestionStats } from '@/composables/useWrongQuestionAnalysis';
import { generateComparisonTutoringPlan } from '@/composables/useTutoringPlan';
import type { ComparisonConfig, AnswerStatus, ScoreResult, ComparisonWrongQuestion, ComparisonWrongQuestionStats, ComparisonTutoringPlan as ComparisonTutoringPlanType } from '@/types';
...
// Reactive state
const config = ref<ComparisonConfig>({ ...defaultConfig });
const showAnswers = ref(false);
const answerMode = ref<'practice' | 'answering'>('practice');

// Use comparison generator composable
...
const wrongQuestions = ref<ComparisonWrongQuestion[]>([]);
const wrongQuestionStats = ref<ComparisonWrongQuestionStats | null>(null);

// Tutoring plan
const tutoringPlan = ref<ComparisonTutoringPlanType | null>(null);
...
// Student answers map for ComparisonDisplay component
...
const handleSubmitAnswers = () => {
  ...
  questions.value.forEach(question => {
    ...
    const isCorrect = validateComparisonAnswer(studentAnswer.answer, question.answer);
  ...
  wrongQuestions.value = extractComparisonWrongQuestions(questions.value, studentAnswers.value);
  
  // Calculate wrong question statistics
  if (wrongQuestions.value.length > 0) {
    wrongQuestionStats.value = calculateComparisonWrongQuestionStats(wrongQuestions.value);
    
    // Generate tutoring plan
    tutoringPlan.value = generateComparisonTutoringPlan(
...
const handleApplyRecommendedConfig = (recommendedConfig: Partial<ComparisonConfig>) => {
...
const handleExportTxt = (includeAnswers: boolean) => {
  ...
  try {
    exportComparisonToTxt(questions.value, includeAnswers);
  } catch (error) {
...
const handleExportPdf = (includeAnswers: boolean) => {
  ...
  try {
    exportComparisonToPdf(questions.value, includeAnswers);
  } catch (error) {
...
const handlePrint = (includeAnswers: boolean) => {
  ...
  try {
    printComparison(questions.value, includeAnswers);
  } catch (error) {
