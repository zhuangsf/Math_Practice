...
// modify by jx: implement geometry question display component with 4-column layout and answering mode support

import { computed, ref, watch } from 'vue';
import type { GeometryQuestion, AnswerStatus } from '@/types';
...
const formatAnswer = (answer: number): string => {
  // modify by jx: format answer for display, preserve precision but remove unnecessary trailing zeros
  // Use toFixed(2) to ensure consistent formatting, then remove trailing zeros
...
const columns = computed(() => {
  const cols: (GeometryQuestion & { index: number })[][] = [[], [], [], []];
