        ...
        >
          <div class="question-content">
            ...
            <template v-if="answerMode === 'practice'">
              <span v-if="showAnswers" class="question-answer">{{ formatFraction(question.answer) }}</span>
          ...
          <div v-if="answerMode === 'answering' && isSubmitted && getAnswerStatus(question.id) === 'wrong'" class="correct-answer-hint">
            正确答案：{{ formatFraction(question.answer) }}
          </div>
        </div>
      </div>
    </div>
...
// modify by jx: implement fraction question display component with 4-column layout and answering mode support

import { computed, ref, watch } from 'vue';
import type { FractionQuestion, AnswerStatus } from '@/types';
import { fractionToString } from '@/utils/fractionUtils';
...
const formatFraction = (fraction: { numerator: number; denominator: number }): string => {
...
const columns = computed(() => {
  const cols: (FractionQuestion & { index: number })[][] = [[], [], [], []];
