...
// modify by jx: implement factor multiple question display component with 4-column layout and answering mode support

import { computed, ref, watch } from 'vue';
import type { FactorMultipleQuestion, AnswerStatus } from '@/types';
...
const props = defineProps<{
  ...
  studentAnswers?: Map<string, { answer: string | number | number[] | null; status: AnswerStatus }>;
  isSubmitted?: boolean;
}>();

// Define emits
const emit = defineEmits<{
  (e: 'answer-change', questionId: string, answer: string | number | number[] | null): void;
}>();
...
const formatAnswer = (answer: number | number[]): string => {
...
const columns = computed(() => {
  const cols: (FactorMultipleQuestion & { index: number })[][] = [[], [], [], []];
